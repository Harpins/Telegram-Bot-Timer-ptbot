...
class Bot():
    ...
    def create_timer(self, timeout_secs, callback, *args, **kwargs):
        ...
        if args:
            raise TypeError(f"create_timer() takes 2 positional arguments but {
                            len(args) + 2} were given")
    ...
    def create_countdown(self, timeout_secs, callback, *args, **kwargs):
        ...
        if not timeout_secs:
            raise TypeError("Не могу запустить таймер на None секунд")
        if args:
            raise TypeError(f"create_countdown() takes 2 positional arguments but {
                            len(args) + 2} were given")
        ...
        def first_callback(context): return callback(timeout_secs, **kwargs)
    ...
    def reply_on_message(self, callback, *args, **kwargs):
        if not callable(callback):
            raise TypeError('Ожидаем функцию на вход')
        if args:
            raise TypeError(f"reply_on_message() takes 1 positional argument but {
                            len(args) + 1} were given")
